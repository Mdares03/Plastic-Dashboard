...
export default function KpiTile({ label, value, caption, emptyCaption, tone = "neutral" }: KpiTileProps) {
  ...
  const valueClass = !hasValue ? "text-zinc-400" : tone === "primary" ? "text-emerald-300" : "text-white";
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
      ...
      {hasValue && caption ? (
        <div className="mt-1 text-[11px] uppercase tracking-wide text-zinc-400">{caption}</div>
      ) : null}
      {!hasValue && emptyCaption ? <div className="mt-1 text-[11px] text-zinc-400">{emptyCaption}</div> : null}
    </div>
  );
}
