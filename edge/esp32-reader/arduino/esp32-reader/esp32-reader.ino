...
static void ensureMqtt() {
  if (mqtt.connected()) return;
  // Throttle reconnect attempts (and their serial logging) to ~2s.
  static int64_t lastAttempt = -100000;
  int64_t now = clk.deviceMs();
  if (now - lastAttempt < 2000) return;
  lastAttempt = now;

  String clientId = "mis-edge-" + gMachineId;
  Serial.printf("MQTT: connecting to %s:%d ...\n",
                provisioned("mqttHost", CFG_MQTT_HOST).c_str(), CFG_MQTT_PORT);
  if (mqtt.connect(clientId.c_str())) {
    ...
    // PubSubClient state(): -4 timeout, -2 connect failed (no route/refused), etc.
...
void setup() {
  ...
  Serial.printf("WiFi: joining SSID '%s' (2.4GHz only); broker target %s:%d\n",
                ssid.c_str(), provisioned("mqttHost", CFG_MQTT_HOST).c_str(), CFG_MQTT_PORT);
}

void loop() {
  // Report WiFi state changes so bench bring-up shows whether the join worked and
  // which subnet the board landed on (must be able to route to the broker host).
  static wl_status_t lastWifi = (wl_status_t)0xFF;
  ...
  if (w != lastWifi) {
    ...
    if (w == WL_CONNECTED) {
      ...
      // 1=NO_SSID_AVAIL, 4=CONNECT_FAILED(bad pass), 6=DISCONNECTED, 0=IDLE
      Serial.printf("WiFi: status=%d (not connected)\n", (int)w);
    }
  }

  if (w == WL_CONNECTED) ensureMqtt();
  mqtt.loop();
